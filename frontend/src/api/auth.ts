...
export const authApi = {
  ...
  setup: (data: SetupRequest) =>
    client.post<ApiResponse<User>>('/auth/setup', data).then(r => r.data),
  updateProfile: (data: Partial<User>) =>
    client.put<ApiResponse<User>>('/auth/profile', data).then(r => r.data),
  changePassword: (data: { currentPassword: string; newPassword: string }) =>
    client.post<ApiResponse<void>>('/auth/change-password', data).then(r => r.data),
};