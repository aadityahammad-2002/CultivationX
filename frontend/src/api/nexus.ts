...
export const nexusApi = {
  // ✅ SAHI: Explicit property name
  connectGitHubWithToken: (token: string) =>
    client.post<ApiResponse<GitHubProfileResponse>>('/nexus/github/token', { token: token }).then(r => r.data),
  ...
  connectLeetCode: (username: string) =>
    client.post<ApiResponse<LeetCodeProfileResponse>>('/nexus/leetcode/connect', { username: username }).then(r => r.data),
  ...
  enableLeetGit: (repoName: string) =>
    client.post('/nexus/leetgit/enable', { repoName: repoName }).then(r => r.data),
